...
@section('title','Emploi Professeurs')
...
<div class="container-fluid">
	<div class="row">
		...
		<h2 class="content-heading">Emploi {{!isset($prof_nom) ? '' : ': '.$prof_nom[0]}}</h2>
		<h2 class="content-description">Emploi du temps</h2>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-3 col-md-3">
			{!! Form::open(['route' => 'Emploi_professeurs' ,'method' => 'post']) !!}
			{{ Form::select('id_classe',$classes )}}<br><br>
		</div>
		<div class="col-lg-3 col-md-3">
			{{ Form::select('id_groupe', $groupes) }}<br><br>
		</div>
		<div class="col-lg-3 col-md-3">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content" style="margin-top:18%">
          ...
          <div class="modal-body">
          	...
          	{!! Form::hidden('id_prof',$profs) !!}
          	{{-- {{ Form::label('Professeurs:', null, ['class' => '']) }}
            {{ Form::select('id_prof',$profs )}}<br><br> --}}
            {{ Form::label('Groupes:', null, ['class' => '']) }}
            {{ Form::select('id_groupe_m', $groupes) }}<br><br>
...
jQuery(document).ready(function($) {
    ...
    $('select[name="id_classe"]').change(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{route("get_groupes_foremploi_prof")}}',
            data: {
                'id_classe': $(this).val(),
            },
            success: function(r) {
                $.each(r[0], function(res) {
                    $('select[name="id_groupe"]').append($("<option />").val(r[0][res].id).text(r[0][res].Nom));
                });
                $.each(r[1], function(res) {
                    $('select[name="id_prof"]').append($("<option />").val(r[1][res].id).text(r[1][res].Nom));
                });
                $('.spinner').hide();
            },
            error: function(r) {
    ...
    $('select[name="id_groupe"]').change(function() {
        $('select[name="id_prof"]').empty();
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $("meta[name='csrf-token']").attr('content')
        ...
        $.ajax({
            type: 'POST',
            url: '{{route("get_profs_bygroupe")}}',
            data: {
                'id_groupe': $(this).val()
            },
            success: function(r) {
                $.each(r, function(res) {
                    $('select[name="id_prof"]').append($("<option />").val(r[res].id).text(r[res].Nom));
                });
            },
            error: function(r) {
                alert('Something went wrong');
    ...
    $('#save-emploi').click(function() {
        ...
        $.ajax({
            ...
            data: {
                'id_groupe': $('select[name="id_groupe_m"]').val(),
            ...
            success: function(r) {
                ...
                $.each($("#emploi tbody tr td"), function(res) {
                    if ($(this).attr('day') == $('input[name="jour"]').val() && $(this).attr('time') == $('input[name="hour_start"]').val()) {
                        $(this).empty();
                        $(this).append("<p><b>Grp: </b>" + r[0].Nom + "</p><p><b>Salle: </b>" + r[1].Nom + "</p>");
                    }
                });
            },
            error: function(r) {
