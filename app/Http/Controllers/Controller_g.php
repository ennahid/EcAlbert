...
class Controller_g extends Controller
{
    ...
    public function Emploi_classes(Request $req)
    {
        ...
        {
            // $emploi = Emploi::where('emploi.id_groupe',$req->input('id_groupe'))->get();
    ...
    public function update_emp(Request $req)
    {
        if($req->input('prof'))
        {
            Emploi::where([['Jour','=',$req->input('jour')],['hour_start','=',$req->input('hour_start')],['id_prof','=',$req->input('id_prof')]])
                                    ->update(['id_prof' => null]);
            ...
            $groupe = Groupes::where('id',$req->input('id_groupe'))->first();
            $salles = salles::where('id',$req->input('id_salle'))->first();
            // return $req->input('id_groupe');
            // return ('prof : '. $req->input('id_prof'). '   grp : '. $req->input('id_groupe').'   hour_start :' . $req->input('hour_start') . '   Jour : '. $req->input('jour'));
            return  array($groupe,$salles);
        }
        else
        {
            Emploi::where([['Jour','=',$req->input('jour')],['hour_start','=',$req->input('hour_start')],['id_groupe','=',$req->input('id_groupe')]])
            ...
            // return  array($req->input('id_matiere'),$req->input('id_groupe'),$salles);
        }
    ...
    public function Emploi_professeurs(Request $req)
    {
        if($req->input('id_classe') != null && $req->input('id_groupe')!= null && $req->input('id_prof')!= null)
        {
            $emploi = Emploi::where('emploi.id_prof',$req->input('id_prof')) 
                    ...
                    ->select('emploi.*','groupes.Nom as grpNom','salles.Nom as salleNom')
                    ->orderby('Jour','asc')
                    ->orderby('hour_start','asc')
                    ->get();
        ...
        $classes = Classes::pluck('Nom','id');
        $salles = salles::pluck('Nom','id');
        $groupes = Groupes::where('id',$req->input('id_groupe'))->pluck('Nom','id');
        $profs = Professeurs::where('id',$req->input('id_prof'))->pluck('Nom','id');
        $groupes = Professeur_Groupe::where('id_professeur',$req->input('id_prof'))
                                    ->join('groupes','groupes.id','=','professeurs_groupes.id_groupe')
                                    ->pluck('groupes.Nom','groupes.id');
